...
var flatten = function(options) {
    ...
    // Check for override
    var override = Object.entries(options.customOverrides || {}).find(function([packageName]) {
        if (packageName.lastIndexOf("@") > 0) {
            return packageName === key;
        } else {
            return packageName === json.name;
        }
    });
    if (override) {
        override = Object.assign({}, override[1]);
        if (override.licenseFile) {
            override.licenseFile = path.resolve(override.licenseFile);
        }
    }
    ...
    if (json.path && fs.existsSync(json.path)) {
        ...
        if (override && override.licenseFile && fs.existsSync(override.licenseFile)) {
            var stat = fs.statSync(override.licenseFile);
            if (stat.isDirectory()) {
                var overrideFiles = fs.readdirSync(override.licenseFile);
                files = licenseFiles(overrideFiles);
            } else {
                files = [path.basename(override.licenseFile)];
                override.licenseFile = path.dirname(override.licenseFile);
            }
        } else {
    ...
    files.forEach(function(filename, index) {
        if (override && override.licenseFile) {
            licenseFile = path.join(override.licenseFile, filename);
        } else {
    ...
    /*istanbul ignore else*/
    if (override) {
        delete override["licenseFile"];
        Object.assign(moduleInfo, override);
    }
...
var loadAdditional = function(options) {
    var data = options.data;

    if (options.deps) {
        Object.keys(options.deps).forEach(function(id) {
            var json = Object.assign({}, options.deps[id]);
            ...
            if (json.licenseFile) {
                var licenseFile = path.isAbsolute(json.licenseFile)
                    ? path.resolve(json.licenseFile)
                    : path.resolve(options.path, json.licenseFile);

                if (fs.lstatSync(licenseFile).isFile()) {
                    var content;
                    if (!json.licenses || json.licenses.indexOf(UNKNOWN) > -1 || json.licenses.indexOf('Custom:') === 0) {
                        // Only re-check the license if we didn't get it from elsewhere
                        content = fs.readFileSync(licenseFile, { encoding: 'utf8' });
                        json.licenses = license(content);
                    }

                    if (include("licenseFile")) {
                        json.licenseFile = options.basePath ? path.relative(options.basePath, licenseFile) : licenseFile;
                    }
                    ...
                    if (include("licenseText") && options.customFormat) {
                        ...
                        if (options._args && !options._args.csv) {
                            json.licenseText = content.trim();
                        } else {
                            json.licenseText = content.replace(/"/g, '\'').replace(/\r?\n|\r/g, " ").trim();
                        }
                    }
                }
            }
...
exports.init = function(options, callback) {
    ...
    if (options.customPath) {
        options.definitions = this.parseJson(options.customPath);
        if (options.definitions.format) {
            options.customFormat = options.definitions.format;
        }
        if (options.definitions.overrides) {
            options.customOverrides = options.definitions.overrides;
        }
        if (options.definitions.failOn && !options.failOn) {
            options.failOn = options.definitions.failOn.join(";");
        }
        if (options.definitions.onlyAllow && !options.onlyAllow) {
            options.onlyAllow = options.definitions.onlyAllow.join(";");
        }
        if (options.definitions.packages && !options.packages) {
            options.packages = options.definitions.packages.join(";");
        }
        if (options.definitions.exclude && !options.exclude) {
            options.exclude = options.definitions.exclude.join(",");
        }
        if (options.definitions.excludePackages && !options.excludePackages) {
            options.excludePackages = options.definitions.excludePackages.join(";");
        }
        if (options.definitions.excludePrivatePackages !== undefined && options.excludePrivatePackages === undefined) {
            options.excludePrivatePackages = options.definitions.excludePrivatePackages;
        }
        if (options.definitions.unknown !== undefined && options.unknown === undefined) {
            options.unknown = options.definitions.unknown;
        }
        if (options.definitions.onlyunknown !== undefined && options.onlyunknown === undefined) {
            options.onlyunknown = options.definitions.onlyunknown;
        }
        if (options.definitions.relativeLicensePath !== undefined && !options.relativeLicensePath) {
            options.relativeLicensePath = options.definitions.relativeLicensePath;
        }
        if (options.definitions.additionalLicenses !== undefined && !options.additionalLicenses) {
            options.additionalLicenses = options.definitions.additionalLicenses;
        }
    }
    var opts = {
        dev: true,
    ...
    read(options.start, opts, function(err, json) {
        ...
        // Include statically defined additional licenses
        data = loadAdditional({
            data: data,
            deps: options.additionalLicenses,
            customFormat: options.customFormat,
            basePath: options.relativeLicensePath ? json.path : null,
            path: json.path,
            _args: options
    ...
    if (customFormat && Object.keys(customFormat).length > 0) {
        Object.keys(sorted).forEach(function sortedCallback(sortedItem) {
            ...
            Object.keys(customFormat).forEach(function customCallback(customItem) {
                if (customItem === "licenseText") {
                    if (sorted[sortedItem][customItem]) {
                        text.push('    ```');
                        text.push('    ' + sorted[sortedItem][customItem].replace(/\n/g, "\n    "));
                        text.push('    ```');
                    }
                } else {
